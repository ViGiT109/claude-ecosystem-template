...
> **Updated:** 2026-05-21 — **v2.0.1 hotfix shipped** on top of v2.0.0.
> Loaded automatically by `session_start.py` hook (first 25 lines).

## Current Focus

**v2.0.0 + v2.0.1 — both SHIPPED.** Six phases of v2.0.0 (1–6b) merged into `main` via fast-forward + v2.0.1 hotfix bundle addressing two P1 audit findings (CHANGELOG numeric, first `audit_complete` row).
...
- [x] Release v2.0.0 — CHANGELOG written, fast-forward merge to main, tag `v2.0.0` cut, pushed to origin, GitHub release published — **DONE**
- [x] Sanity audit on released state — `ecosystem-auditor` subagent → 🟢 **92/100** (target ≥90 hit) — **DONE**
- [x] v2.0.1 hotfix — two P1 audit findings (CHANGELOG numeric, first `audit_complete` row) — **DONE**
...
- **2026-05-21:** **v2.0.1 hotfix shipped** on `fix/v2.0.1-audit-followups` (off `main` at v2.0.0). Two P1 findings from the post-release sanity audit addressed: (a) CHANGELOG Phase 3 bullet read "9 slash commands" — live count is 10 (8 existing + 2 new); fixed to "10 slash commands (8 existing + 2 new)". (b) `.memory/audit_history.jsonl` had no prior `audit_complete` row, so the v2.0.0 Phase-2 freshness fix lacked a production datapoint; appended the first `audit_complete` row (score 92, mode cross-session, source ecosystem-auditor subagent, findings list of all 5 audit P-items). v2.0.1 CHANGELOG section added; `task.md` rewritten as a v2.0.1 follow-up checklist; remaining P2/P3 findings explicitly deferred to a v2.1 backlog (hook smoke-test harness, ChromaDB happy-path verification, prose-vs-manifest counter check, logger adoption in scripts/).
